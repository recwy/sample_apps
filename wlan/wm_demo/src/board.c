/*
 *  Copyright (C) 2008-2015, Tapu Instrument Manufacturing Co,Ltd.
 *  All Rights Reserved.
 */

/*
 * This is a board specific configuration file for
 * 88MC200+88W8801 board. The name/version is printed on
 * the board.
 * This board is based on 88MC200+88W8801 Rev:1.0
 * schematics
...
#error You have not selected Wi-Fi 8801 based chipset. Please use "make mc200_8801_defconfig" to select 8801-based Chipset.
#endif
...
int board_button_pressed(int pin)
{
    ...
    if(pin == GPIO_25)
        GPIO_PinMuxFun(pin, PINMUX_FUNCTION_1);
    else
...
void board_gpio_power_on()
{
	...
	PMU_ConfigVDDIOLevel(PMU_VDDIO_SDIO, PMU_VDDIO_LEVEL_3P3V);
...
int board_button_3()
{
	GPIO_PinMuxFun(GPIO_29, PINMUX_FUNCTION_0);
	GPIO_SetPinDir(GPIO_29, GPIO_INPUT);
...
int board_button_4()
{
	GPIO_PinMuxFun(GPIO_25, PINMUX_FUNCTION_1);
